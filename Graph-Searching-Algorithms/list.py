...
- contains(id) returns true if the node with the input id is is in the list and false otherwise.
    ...
    def __shiftDown(self, parent):
        ...
        if left_child <= last_child:
            if right_child <= last_child:
                if self._data[left_child].priority < self._data[right_child].priority:
                    if self._data[parent].priority > self._data[left_child].priority:
                        self.__swap(parent, left_child)
                        self.__shiftDown(left_child)
                else:
                    if self._data[parent].priority > self._data[right_child].priority:
